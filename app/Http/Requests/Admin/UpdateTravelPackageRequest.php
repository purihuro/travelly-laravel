...
class UpdateTravelPackageRequest extends FormRequest
{
    ...
    {
        $travelPackageId = $this->route('travel_package')?->id ?? $this->route('travelPackage')?->id;

        return [
            'slug' => ['required', 'string', 'max:150', Rule::unique('travel_packages', 'slug')->ignore($travelPackageId)],
